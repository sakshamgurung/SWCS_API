        ...
        try {
            this.transactionResults = await session.withTransaction(async() => {
                ...
                if(!_.isEmpty(tempCustomerUsedGeoObject)){
                    ...
                    if(!_.isEmpty(updateUsedTrack)){
                        ...
                        if(!_.isEmpty(removeSchedule)){
                            this.result = await Schedule.deleteById(removeSchedule[0]._id, session);
                    ...
                    if(_.isEmpty(tempCustomerUsedGeoObject[0].usedTrack)){
                        ...
                        this.result = await CustomerUsedGeoObject.deleteById(customerUsedGeoObjectId, session);
                        checkForWriteErrors(this.result, "none", "Subscription delete failed");
                    }else{
                        //update customerUsedGeoObject
                        const {usedTrack} = tempCustomerUsedGeoObject[0].usedTrack;
                        this.result = await CustomerUsedGeoObject.updateById(customerUsedGeoObjectId, {usedTrack}, session);
                        checkForWriteErrors(this.result, "none", "Subscription delete failed");
                    }
                }
                ...
                for(let n of removeNotification ){
                    this.result = await Notification.deleteById(n._id, session);
                    checkForWriteErrors(this.result, "none", "Subscription delete failed");
                }
                
                //delete subscription
                this.result = await Subscription.deleteById(id, session);
                checkForWriteErrors(this.result, "none", "Subscription delete failed");
