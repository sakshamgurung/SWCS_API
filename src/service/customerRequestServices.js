...
class CustomerRequestServices {
	...
	async getAllCustomerRequest(role, id, query) {
		this.result = await CustomerRequest.findAll(role, id, query).populate("companyId", "email mobileNo").populate("customerId", "email mobileNo").populate("staffGroupId").populate("vehicleId");
		this.result = customerRequestArrayServerToClient(this.result);
		return this.result;
	}

	async getCustomerRequestById(id) {
		this.result = await CustomerRequest.findById(id).populate("companyId", "email mobileNo").populate("customerId", "email mobileNo").populate("staffGroupId").populate("vehicleId");
		this.result = customerRequestServerToClient(this.result);
		return this.result;
	}

	async getCustomerRequestByRef(ref, id, query) {
		this.result = await CustomerRequest.findByRef(ref, id, query).populate("companyId", "email mobileNo").populate("customerId", "email mobileNo").populate("staffGroupId").populate("vehicleId");
		this.result = customerRequestArrayServerToClient(this.result);
		return this.result;
	}
